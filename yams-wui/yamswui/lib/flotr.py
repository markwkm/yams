...
class Flotr:
    ...
    def code_button(self):
        # The padding with <br> is to make sure there is enough space between
        # plots.  The long X-axis labels wrap such that the text goes beyond
        # the canvas boundries.
...
%s
            ...
            $('%s').observe('flotr:select', function(evt) {
                ...
                f = drawGraph({
                    ...
                    yaxis: { %s min: area.y1, max: area.y2 }
                });
            });
            ...
            $('%s').observe('click', function() { drawGraph() });
                ...
                self.div_name, self.ylabel, self.button_name)
